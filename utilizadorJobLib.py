...
def prever_categoriaEnsemble(tarefa, threshold=0.45):
    ...
    try:
        ...
        return label_encoder_modelo.inverse_transform([encoded])[
            0
        ]
...
def prever_categoriaNB(tarefa, threshold=0.45):
    ...
    try:
        ...
        encoded = modelo.predict([tarefa])[0]
        return label_encoderNB.inverse_transform([encoded])[
            0
        ]
...
for task in test_cases:
    ...
    print(f"Tarefa: '{task[:30]}...' → Categoria NB: {categoriaNB} ;  → Categoria Ensemble: {categoriaEnsemble} ")    